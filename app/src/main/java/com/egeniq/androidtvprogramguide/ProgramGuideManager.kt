...
class ProgramGuideManager<T> {
    ...
    internal fun getProgramIndexAtTime(channelId: Int?, time: Long): Int {
     ...
     * given `channelId`.
     */
    internal fun getSchedulesCount(channelId: Int): Int {
     ...
     */
    fun getChannel(channelIndex: Int): ChannelUtils.Channel? {
    ...
    fun getChannelIndex(channelId: Int): Int? {
        val index = channels.indexOfFirst { it.number == channelId }
        return if (index < 0) {
            null
        } else {
            index
