...
class ProgramGuideRowGridView @JvmOverloads constructor(
    ...
    var channel: ChannelUtils.Channel? = null
        private set
    ...
    fun setChannel(channelToSet: ChannelUtils.Channel) {
    ...
    fun resetScroll(scrollOffset: Int) {
        ...
        val position = if (channel == null) {
            ...
            programGuideManager.getProgramIndexAtTime(channel.number, startTime)
        }
        if (position < 0) {
            layoutManager?.scrollToPosition(0)
        } else if (channel?.number != null) {
            val slug = channel.number
